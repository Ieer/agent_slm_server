...
class EmbeddingDataItem(BaseModel):
    ...
    index: int
    embedding: list[float] | str  # 若 encoding_format=base64 則為字串
    model: str | None = None
...
class EmbeddingAPIClient:
    ...
    def create_embeddings(
        ...
        # 標準情況：一次性處理所有文本，並且在記憶體錯誤時嘗試自動降級
        try:
            ...
            if any(ind in err_lower for ind in memory_error_indicators):
                if model != self.fallback_model:
                    print(f"[自動降級] 從 {model} 切換到 {self.fallback_model}")
                    return self._call_embedding_api(input_texts, self.fallback_model, encoding_format)
            raise
    ...
    def calculate_similarity(self, query: str, documents: list[str], model: str | None = None) -> list[float]:
        """
        計算查詢和文檔之間的餘弦相似度
